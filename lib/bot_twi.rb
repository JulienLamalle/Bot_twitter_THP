...
def tweets_to_journalists(client)
  ...
  list_of_handle = journalists.sample(5)
  list_of_handle.each do |x|
    client.update(" #bonjour_monde , Merci pour le travail que tu fais #{x} , et longue vie à @the_hacking_pro ")
  end
end
...
tweets_to_journalists(client)

=end

def search_for_me(client)
  client.search(" #bonjour_monde -rt ").take(25).each do |x|
    client.favorite(x)
  end
end

=begin
search_for_me(login_twitter)
=end

def follow_for_me(client)
  client.search(" #bonjour_monde ").take(25).each do |x|
    if x.user.name != "Julien Lamalle"
      puts x.user.name
      client.follow(x.user) 
    end
  end
end
=begin
follow_for_me(login_twitter)
=end

def stream_for_me(client_streaming)
  client_streaming.filter(track: "#bonjour_monde").each do |x|
  client_streaming.favorite(x)
  client_streaming.follow(x.user)
  end
end

stream_for_me(login_twitter_streaming)